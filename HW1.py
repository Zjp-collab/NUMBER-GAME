...
def rectangle(perimeter,area):
    ...
    # Check if sqrt_discriminant is integer
    if not sqrt_discriminant.is_integer():
        return False
    
    # Possible sides
    x1 = (half_p + sqrt_discriminant) / 2
    x2 = (half_p - sqrt_discriminant) / 2
...
def to_decimal(oct_num):
    ...
    power = 0
    while oct_num > 0:
        digit = oct_num % 10          # extract last digit
        decimal += digit * (8 ** power)  # multiply by 8^power
        oct_num = oct_num // 10       # remove last digit
        power += 1
    return decimal
...
def has_hoagie(num):
    ...
    while num >= 100:
        ...
        num = num // 10  # shift window one digit to the right
...
def is_identical(num_1, num_2):
    ...
    def collapse(num):
        compressed = 0
        place = 1 
        prev_digit = -1  # impossible digit
        
        while num > 0:
            digit = num % 10
            if digit != prev_digit:
                compressed += digit * place
                place *= 10
                prev_digit = digit
            num = num // 10
        return compressed
    
    return collapse(num_1) == collapse(num_2)
...
def successors(file_name):
    ...
    for token in tokens:
        if prev not in result:
            ...
            if token not in result[prev]:
                result[prev].append(token)
        prev = token
...
def wordExists(trie, word):
    ...
    for ch in word:
        if ch not in current:
            return False
        current = current[ch]
    return 'word' in current and current['word']
    pass
