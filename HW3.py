    ...
    # ---------------- TOKENIZATION (INLINE) ----------------
        ...
        while i < len(txt):
            ...
            if ch.isspace():
                ...
                if ch.isdigit() or ch == '.' or (ch == '-' and (i == 0 or txt[i-1] in '+-*/^(')):
                    num = ch
                    i += 1

                    while i < len(txt) and (txt[i].isdigit() or txt[i] == '.'):
                        num += txt[i]
                        i += 1
        ...
        for token in tokens:
            ...
            if self._isNumber(token):
                ...
                while (not postfixStack.isEmpty() and
                    postfixStack.peek() in precedence and
                    (
                        precedence[postfixStack.peek()] > precedence[token] or
                        (precedence[postfixStack.peek()] == precedence[token] and token not in right_assoc)
                    )):
                    output.append(postfixStack.pop())
